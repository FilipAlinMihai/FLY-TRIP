...
session_start();
	...
	$tip=$_POST["tip"];
	echo adauga_header('Adaugare postare FlyTrip');
	$cPAtr = array(
	      "page_title" => "Adăugare Postare",
		  "site_title" => "FlyTrip",
		  "page_description" => $locatie,
		  "legaturi" => array (
			array ( 
					"nume" => "Prima Pagină",
	...
	echo adauga_meniu($cPAtr).'<div class="center">'; 
    ...
    $postare="Insert into `postare` values ('".$locatie."','".$_SESSION['email']."','".$tip."','".$descriere."',".$codul.",'".$imagine."','".$imagine2."','".$imagine3."','".$d."')";
		if(mysqli_query($b,$postare)):
			echo '<p class="error">Postarea a fost adăugată</p>';
		else:
			echo '<p class="error">Procesul eşuat'. mysqli_errno($b). ' : '. mysqli_error($b).'</p>';
		endif;
